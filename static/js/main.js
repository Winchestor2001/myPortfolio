    ...
    $('#contact-form').on('submit', function(e) {
        ...
        if( required === 0 ) {
            $.ajax({
                ...
                error: function(data) {
                    showAlertBox(data.status, 'wasn\'t');
                }
            });
        }
    });
...
function showAlertBox(response, message) {
    ...
    if( response == 200 ) {
        $alertBox.addClass('alert-success').html(`Message was sent`);
        $alContainer.html($alertBox);
    } else {
        $alertBox.addClass('alert-danger').html(`Message wasn't sent`);
        $alContainer.html($alertBox);
    }
